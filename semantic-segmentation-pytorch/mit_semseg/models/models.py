...
class SegmentationModule(SegmentationModuleBase):
    ...
    def forward(self, feed_dict, *, segSize=None):
        ...
        if segSize is None:
            ...
            if seg_label.shape[-2:] != pred.shape[-2:]:
                # Resize target to match prediction spatial size (nearest neighbor to keep integer labels)
                seg_label = F.interpolate(seg_label.unsqueeze(1).float(), size=pred.shape[-2:], mode='nearest').squeeze(1).long()
